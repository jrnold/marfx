...
mod_pfx <- fdfx(mod, data1 = data1, data2 = data2)
...
Since `amfx` calculates the average marginal effect, we need to subset the Duncan data, run `amfx` and recombine it into the dataset that we can use for analysis.
```{r}
.data <- list()
for (lvl in levels(Duncan$type)) {
  .data[[lvl]] <-
    amfx(mod, "education", data = filter(Duncan, type == lvl)) %>%
    as.data.frame() %>%
    mutate(type = lvl)
}
.data <- bind_rows(.data)
