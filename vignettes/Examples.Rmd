...
### COMMENT: This seems to be a different paper? Not sure what this has to do with the nationalist vote share plots below.
...
### COMMENT: This is a little confusing because you use two different methods on two different marginal effects plots. Might be good to have each plot model the same variable (changenonslavs or slavicshare) so that you can see the different in output. It might also be good to explain the separate advantages of the conf.int method and the  st.err. method, as well as to mention that mfx generates the confidence intervals by simulation to explain why it returns a different result. My guess is that not everyone who uses the package will have read King et al.
...
### COMMENT: These functions are doing something odd. The average marginal effects of age for the two code blocks above return different results. The first block returns results for "age" that match the results for "educate" from the second code block. I'm not sure where the error is coming from, but it seems like a problem.
...
```
### Comment: The output of this function makes it unclear which entry is related to which value of "race"
...
afdfx(mod_turnout,
      ...
      data2 = mutate(turnout, race = "others"))
```
