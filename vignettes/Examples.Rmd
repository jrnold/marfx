...
as.data.frame(mod_pfx) %>%
  ...
  ggplot(aes(x = type, y = estimate, ymin = lwr, ymax = upr)) +
  geom_pointrange() +
  coord_flip()
```
...
ggplot(.data, 
       aes(colour = type, x = education, y = estimate, ymin = lwr, ymax = upr)) +
  geom_pointrange()
...
ggplot() + 
  geom_ribbon(data = .data, mapping = aes(fill = type, x = education, ymin = lwr, ymax = upr), alpha = 0.3) + 
  geom_line(data = .data, mapping = aes(color = type, x = education, y = estimate)) +
...
.data <- marfx(mod2, "education", data = data_frame(education = seq(0, 100, by = 10))) %>%
  as.data.frame() %>%
  mutate(education = seq(0, 100, by = 10))
  
ggplot() + 
  geom_ribbon(data = .data, mapping = aes(x = education, ymin = lwr, ymax = upr), alpha = 0.3) + 
